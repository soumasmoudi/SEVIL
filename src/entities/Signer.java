...
import databases.DatabaseSigner;
...
public class Signer {
	...
	private final DatabaseSigner databaseSigner;
	...
	private List<MessageSigned> message_list;
	...
	public Signer(TrustedAuthority ta, GroupManager gm) {
		...
		/**	Join_SGr Algorithm STEPs 4 to 9	 */
		...
		this.databaseSigner = gm.join_SGr(pkp);
	...
	/** G_Sign Algorithm with multithreading **/

	public Message G_sign(Element M) {
		...
		Element rho1 = databaseSigner.getRho1();
		Element tau1 = databaseSigner.getTau1();
		Element gamma2z = databaseGM.getGamma2z();
		Element zeta1 = databaseSigner.getZeta1();
		Element z1 = databaseSigner.getZ1();
		...
		Element z2 = databaseSigner.getZ2();
		Element rho2 = databaseSigner.getRho2();
		Element tau2 = databaseSigner.getTau2();
		Element gamma1z = databaseGM.getGamma1z();
		Element zeta2 = databaseSigner.getZeta2();
		...
		return new Message(M, pi, statements);

	}
	
	public void add_message(MessageSigned message) {
		message_list.add(message);
	}

	public List<MessageSigned> getMessage_list() {
		return message_list;
	}
